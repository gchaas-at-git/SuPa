...
from pwfunctions import neighbor_chars
...
if __name__ == "__main__":
    ...
    with tab1:
        ...
        # Create a sidebar 
        # to avoid restarting the app upon changing an input without pressing submit, put everthng in a form
        with st.form("form"):

            # Button to show or hide the download buttons
            if st.form_submit_button("create passwords"):
        ...
        if st.session_state.show_buttons:
            ...
            while i < input_num_pw:
                ...
                if find_consecutive_chars(pw) < 0.5 and neighbor_chars(pw) < 0.5:
                    try:
                        ls.index(pw)
                    except ValueError:
            ...
            #download buttons
            st.sidebar.write(f"**{len(df)} passwords were generated!**")
            st.sidebar.write("Your passwords look like this:")    
            st.sidebar.write(df.head())
                #st.write(tabulate(df[0:6], headers='keys', tablefmt='psql', showindex=False))
            st.sidebar.write("You can download your passwords the settings as a csv:")
